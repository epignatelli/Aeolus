...
def line_srf_intersection(p0, p1, pl_pts, epsilon=1e-6):
    ...
    t0 = np.dot((p0 - p_co),p_no)
    t1 = np.dot((p1 - p_co),p_no)


    if (abs(dot) > epsilon) and (t0*t1 < 0): #parallel condition + segment plane intersection
...
pls_pts,src_pts,tgt_pts = import_geo()
pl_pts = pls_pts[0]


p0 = [-5.3811204723,-1.50146301665,10.4178278611]
p1 = [-6.12950263274,-6.43748486337,15.4378321417]


inter = line_srf_intersection(p0, p1, pl_pts, epsilon=1e-6)
